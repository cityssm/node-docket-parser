...
export function parseDockets(docketFileText: string): Docket[] {
  ...
  for (
    docketLineIndex = 0;
    docketLineIndex < docketLines.length;
    docketLineIndex += 1
  ) {
    ...
    if (
      ...
      while (docketLineIndex < docketLines.length) {
        docketLineIndex += 1
        const itemLine1 = (docketLines.at(docketLineIndex) ?? '').padEnd(
          spacePaddingLength,
          ' '
        )
        ...
        docketLineIndex += 1
        const itemLine2 = (docketLines.at(docketLineIndex) ?? '').padEnd(
          spacePaddingLength,
          ' '
        )
        ...
        const informationNumber = (
          itemLine1.slice(5, 16).trim() +
          ' ' +
          itemLine2.slice(5, 16)
        ).trim()
